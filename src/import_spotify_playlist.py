...
def import_spotify_playlist(playlist_url, journey_id=None, granularity="Track"):
    ...
    with engine.connect() as connection:
        ...
        try:
            ...
            """)
            connection.execute(journey_query, {"jid": journey_id, "jname": playlist_name, "jdesc": playlist_desc, "creator": creator_name, "gran": granularity})
            logger.info(f"Journey '{playlist_name}' ({journey_id}) imported with granularity '{granularity}' and creator '{creator_name}'.")
            ...
            """)
            connection.execute(playlist_query, {
                "jid": journey_id,
            ...
            def get_or_create_performer(artist):
                ...
                logger.info(f"Upserted performer: {name} (ID: {res[0]}) with role {role_val}")
                return res[0]

            # Helper: get or insert album
            def get_or_create_album(album, performer_id):
                title = album['name']
                spotify_url = album['external_urls']['spotify'] if 'external_urls' in album and 'spotify' in album['external_urls'] else None
                ...
                logger.info(f"Upserted album: {title} (ID: {res[0]}) performer {performer_id}")
                return res[0]

            # Track unique albums for album-level journeys
            album_keys = set()
            step_order = 1
            result = connection.execute(text("SELECT MAX(JourneyStepID) FROM FactJourneyStep"))
            max_id = result.scalar()
            try:
            ...
            for item in tracks:
                track = item['track']
                logger.info(f"Processing track: {track.get('name', 'Unknown')} | Artists: {[a['name'] for a in track.get('artists', [])]}")
                ...
                logger.info(f"Album API: {album_api.get('name', 'Unknown')} | Spotify ID: {album_id_spotify}")
                ...
                if granularity == "Album":
                    ...
                    if album_key in album_keys:
                        logger.info(f"Skipping duplicate album step: {album_api['name']} (ID: {album_id}) performer {performer_id}")
                        continue
                    album_keys.add(album_key)
                    step_params = {"jid": journey_id, "order": step_order, "aid": album_id}
                    ...
                    try:
                        ...
                        if getattr(result, 'rowcount', None) == 0:
                            logger.info(f"CONFLICT: Dummy record inserted for params={step_params}")
                        else:
                            logger.info(f"Upserted album step: params={step_params}, rowcount={getattr(result, 'rowcount', 'N/A')}")
                    except Exception as e:
                        logger.error(f"Failed to upsert album step: params={step_params}, error={e}")
                else:
                    step_params = {"jid": journey_id, "order": step_order, "rid": track['id']}
                    ...
                    try:
                        result = connection.execute(step_query, step_params)
                        logger.info(f"Upserted track step: params={step_params}, rowcount={getattr(result, 'rowcount', 'N/A')}")
                    except Exception as e:
                        logger.error(f"Failed to upsert track step: params={step_params}, error={e}")
                step_order += 1
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Import a Spotify playlist into the music journey database.")
    parser.add_argument("playlist_url", help="Spotify playlist URL")
    parser.add_argument("--journey-id", help="Journey ID to use (default: derived from playlist)")
    parser.add_argument("--granularity", choices=["Track", "Album"], default="Track", help="Step granularity: Track or Album (default: Track)")
    args = parser.parse_args()
    import_spotify_playlist(args.playlist_url, journey_id=args.journey_id, granularity=args.granularity)
